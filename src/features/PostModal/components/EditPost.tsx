...
interface EditPostProps {
  ...
  appData: AppData;
  handleSubmit: (
    formData: EventPostStringifedAuthor | BlogPostStringifiedAuthor
  ) => void;
}
...
export default function EditPost({
  ...
  const [formData, setFormData] = useState<
    EventPostStringifedAuthor | BlogPostStringifiedAuthor
  >({
    id: "",
    title: "",
    description: "",
  ...
  useEffect(() => {
    ...
    if (post && appData.users) {
      const highestStudsYear: number = Math.max(
        ...appData.users.map((user) => user.studsYear)
      );
      const defaultAuthor =
        appData.users.find((user) => user.id === appData.userDetails?.id) ||
        appData.users.find((user) => user.studsYear === highestStudsYear) ||
        appData.users[0];
      ...
      switch (e.target.name) {
        case "pictures": {
          if (e.target.files !== null) {
            ...
            setFormData({
              ...formData,
              pictures: [...formData.pictures, ...urls],
            });
      ...
      switch (target.name) {
        case "author":
          setFormData({
            ...
            author: target.value,
            studsYear:
              appData.users?.find((user) => user.id === target.value)
                ?.studsYear || 0,
          });
          break;
        case "studsYear":
  ...
  const years = (appData.users || [])
    .map((user) => user.studsYear)
  ...
  return (
    ...
    >
      <Modal.Header closeButton className="py-2 text-gray-700">
        <Modal.Title>
          {data.post.title
            ? t(`${data.type}.edit.label.editing`) + ": " + data.post.title
            : t(`${data.type}.edit.label.creating`)}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
          <Form.Group className="mb-3" controlId="formPublished">
            <Form.Check
              type="switch"
              label={t(`${data.type}.edit.label.published`)}
              name="published"
          ...
          <Form.Group className="mb-3" controlId="formBasicEmail">
            <FloatingLabel
              ...
              label={t(`${data.type}.edit.label.author`)}
            >
              <Form.Control
                as="select"
                name="author"
          ...
          <Form.Group className="mb-3" controlId="formStudsYear">
            <FloatingLabel
              controlId="floatingStudsYearSelect"
              label={t(`${data.type}.edit.label.studsYear`)}
            >
              <Form.Control
                as="select"
                name="studsYear"
          ...
          <Form.Group className="mb-3" controlId="formEventDate">
            <FloatingLabel label={"Event date"}>
              <Form.Control
                type="date"
                name="eventDate"
                value={
          ...
          <Form.Group className="mb-3" controlId="formPostTitle">
            <FloatingLabel label={t(`${data.type}.edit.label.title`)}>
              <Form.Control
                type="text"
                placeholder={t(`${data.type}.edit.label.title`)}
                name="title"
          ...
          <Form.Group className="mb-3" controlId="formFileMultiple">
            <Form.Label>
              {t(`${data.type}.edit.label.addFrontImage`)}
            </Form.Label>
            <Form.Control
              ref={addFrontImageRef}
              type="file"
              name="frontPicture"
              onChange={(e) => handleChange(e)}
            />
            <div className="d-flex my-3">
              {formData.frontPicture && (
                <AddedImage
                  isFrontPicture
                  picture={formData.frontPicture}
                  index={0}
                  handleDeleteImage={handleDeleteFrontPicture}
                  imageRefs={frontImageRefs}
                  iconRefs={frontIconRefs}
                />
              )}
            </div>
          </Form.Group>
          ...
          <Alert className="my-3" variant="success">
            ...
            <p>{t(`${data.type}.edit.alertDescription`)}</p>
            <hr />
            <p className="mb-0">

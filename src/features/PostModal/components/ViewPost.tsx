...
export default function ViewPost({
  ...
  if (
    data.type === "blogPost" &&
    appData.userDetails?.permissions.includes(Permission.Blog)
  ) {
    toolsToShow = { edit: true, delete: true };
  } else if (
    data.type === "eventPost" &&
    appData.userDetails?.permissions.includes(Permission.Events)
  ) {
    toolsToShow = { edit: true, delete: true };
      ...
      <Modal.Header closeButton className="px-4 py-3 text-gray-700">
        ...
        <Modal.Title>
          {(toolsToShow.edit || toolsToShow.delete) && (
            <Tools
              id={post.id}
              handleClickEdit={handleClickEditAndClose}
              handleClickDelete={handleClickDeleteAndClose}
              inline={true}
              opacity={1}

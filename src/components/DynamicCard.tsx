...
export default function DynamicCard({
  ...
  return (
    <div className="col">
      <div
        className="card card-cover h-100 overflow-hidden rounded-4 cursor-pointer"
        style={{ transitionDuration: "0.2s", cursor: "pointer" }}
        onClick={(e) => {
          e.stopPropagation();
      ...
      >
        <div
          ref={imageRef}
          className="d-flex flex-column bg-dark h-100"
          style={{
            transitionDuration: "0.2s",
            backgroundImage: `url(${element.bgImg})`,
        ...
        />
        <div
          className="d-flex flex-column h-100 p-3 p-lg-5 pb-lg-3 text-light text-shadow-1"
          style={{ position: "relative" }}
        >
          <div className={`pt-5 ${element.danger ? "mt-3" : "mt-5"} mb-4`}>
            {element.danger && (
              <small className="lead text-warning">{element.danger}</small>
            )}
            {element.cardTitle ? (
              <div className={`display-6 lh-1 fw-bold px-1`}>
                {element.cardTitle}
              </div>
            ) : (
              <div
                className={`display-6 lh-1 fw-bold text-warning ${getDescriptionSize(
                  windowWidth
                )}`}
              >
                {t("card.noTitle")}
